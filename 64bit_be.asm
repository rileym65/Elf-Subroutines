...
; ***** Numbers in memory stored MSB first   *****
; ************************************************
add64:   ldi      7                 ; need to move to lsb
         plo      re
add64_1: inc      r7                ; move first pointer 
         inc      r8                ; move second pointer
         dec      re                ; decrement count
         glo      re                ; see if done
         lbnz     add64_1           ; loop back if not
         ldi      8                 ; 8 bytes to process
         plo      re
         sex      r7                ; point x to first number
         adi      0                 ; clear DF
add64lp: ldn      r8                ; get byte from second number
         adc                        ; add byte of first number
         stxd                       ; store it
         dec      r8
         dec      re                ; decrement count
         glo      re                ; see if done
         ...
         inc      r8                ; restore consumed registers
         inc      r7
...
; ***** Numbers in memory stored MSB first   *****
; ************************************************
sub64:   ldi      7                 ; need to move to lsb
         plo      re
sub64_1: inc      r7                ; move first pointer 
         inc      r8                ; move second pointer
         dec      re                ; decrement count
         glo      re                ; see if done
         lbnz     sub64_1           ; loop back if not
         ldi      8                 ; 8 bytes to process
         plo      re
         sex      r7                ; point x to first number
         smi      0                 ; set DF for first subtract
sub64lp: ldn      r8                ; get byte from second number
         sdb                        ; subtract byte of first number
         stxd                       ; store it
         dec      r8
         dec      re                ; decrement count
         glo      re                ; see if done
         lbnz     sub64lp           ; loop until all bytes processed
         sex      r2                ; restore stack
         inc      r8                ; restore consumed registers
         inc      r7
         sep      sret              ; return to caller
...
inc64:   inc      rf                ; move to lsb
         ...
         ldi      8                 ; 8 bytes to process
         plo      re
         smi      0                 ; set DF for first add
         sex      rf                ; point X to RF
inc64lp: ldn      rf                ; get byte from number
         adci     0                 ; propagate carry
         stxd                       ; store byte
         dec      re                ; decrement count
         glo      re                ; see if done
         lbnz     inc64lp           ; loop back if not doen
         inc      rf                ; move rf back to msb
...
dec64:   inc      rf                ; move to lsb
         ...
         ldi      8                 ; 8 bytes to process
         plo      re
         adi      0                 ; clear DF for first subtract
         sex      rf                ; point X to RF
dec64lp: ldn      rf                ; get byte from number
         sdbi     0                 ; propagate carry
         stxd                       ; store byte
         dec      re                ; decrement count
         glo      re                ; see if done
         lbnz     dec64lp           ; loop back if not doen
         inc      rf                ; move rf back to msb
         sex      r2                ; point x back to stack
         sep      sret              ; and return to caller
...
cmp64:   push     r7                ; save registers
         ...
         stxd
         ldi      7                 ; need to move pointers to lsb
         plo      re
cmp64_1: inc      r7                ; move first number pointer
         inc      r8                ; move second number pointer
         dec      re                ; decrement count
         glo      re                ; see if done
         lbnz     cmp64_1           ; loop back if not done
         ldi      0                 ; clear zero test
         plo      re
         ldi      8                 ; 8 bytes to process
         plo      r9
         smi      0                 ; set DF for first subtract
cmp64lp: ldn      r8                ; get byte from second number
...
icmp64:  push     r7                ; save registers
         ...
         ldi      7                 ; need to move pointers to lsb
         plo      re
icmp641: inc      r7                ; move first number pointer
         inc      r8                ; move second number pointer
         dec      re                ; decrement count
         glo      re                ; see if done
         lbnz     icmp641           ; loop back if not done
         ldi      0                 ; clear zero test
         plo      re
         ldi      8                 ; 8 bytes to process
         plo      r9
         smi      0                 ; set DF for first subtract
icmp64p: ldn      r8                ; get byte from second number
         str      r2                ; store for subtract
         ldn      r7                ; get byte from first number
         sdb                        ; subtract
         str      r2                ; store to combine with zero test
...
norm64:  ldi      0                 ; signal signs are the same
         ...
         dw       comp2s
nrm64n1: ldn      r8                ; get msb of second number
         shl                        ; is it negative
...
scmul4:  inc      r8                ; move to lsb
         ...
         inc      r8
         inc      r8
         ldn      r8                ; get lsb of second number
         dec      r8                ; put r8 back
         dec      r8
         dec      r8
         dec      r8
